...
def aux_operacion_entre_miembros(cadena): # Opera términos semejantes 2 miembros
    ...
    for i in range(len(miembro_2)):
        ...
        terminos_orden.append( miembro_2[ i ]  )
        signos_orden.append(  signo_2[ i ] )
    
    final_reducido_terminos , final_reducido_signos = aux_operacion_entre_terminos(terminos_orden , signos_orden)

    return final_reducido_terminos , final_reducido_signos
...
def aux_ordenar_ecuación_final_float(cadena): # Devuelve la ecuación ya corregida
    ...
    return aux_corregir_espacios_float(cadena_obtenida)
...
# solo de ser necesario, pero para que el usuario entienda mejor, se pone como nombre 'encontrar_constantes'
    #cadena = aux_ordenar_ecuación_final_float(cadena)
...
def aux_encontrar_constantes_recta_float(cadena):
# Realmente se encarga de encontrar los signos de las constantes y corregirlos con la funcion anterior
# solo de ser necesario, pero para que el usuario entienda mejor, se pone como nombre 'encontrar_constantes'
...
def Recta2(a , b , mx , my):
    ...
    return aux_ordenar_ecuación_final_float(cadena).lstrip('1' or '-1')
...
def Distancia_recta(a , b , cadena): # Devuelve valor numérico
    ...
    A , B , C = aux_encontrar_constantes_recta(aux_ordenar_ecuación_final_float(cadena))
...
def Circunf2(a , b , ap , bp , m , n): # Devuelve centro h, k , r
    from clase_geo_Gral import Punto

    """ Muy importante que la funcion Circunf1 del final le llame a la Cfunción
...
def coeficiente_cuadratica(parabola):
    txt = ''
    for i in range(len(parabola)):
        caracter = parabola[ i ]

        if ('+' or '-') not in caracter:
            txt += caracter
...
def para_encontrar_constantes_CDE(cadena): # Devuelve constantes Dx , Ey , F

    h , k , p , fx , fy , bandera = Foco_parabola(cadena)
    #print(f'Parámetro: {p*2}\n')

    if bandera == 'x2':
        a = aux_operando_x2(coeficiente_cuadratica(cadena).upper())

        if p < 0:

            c , d , e = float(-2*h)*a , float(-2*(p))*a , float(h**2 + 4*(p/2)*k)*a
        ...
        b = aux_operando_y2(coeficiente_cuadratica(cadena).upper())

        if p < 0:

            c , d , e = float(-2*p)*b , float(-2*(k))*b , float(k**2 + 4*(p/2)*h)*b
...
def Tangente_parabola(cadena , usuario_x , usuario_y):
    ...
    #cadena = 'y²-4x+8y+44=0' # x2-4x-8y+44=0 , y2-4y+4=-3x-21
    h , k , p , fx , fy , bandera = Foco_parabola(cadena)
    #usuario_x , usuario_y = 11 , -8

    if bandera == 'x2': # Funciona perfectamente

        if p < 0:

            # Se encarga de formar la ecuación de la directriz de la parábola
            directriz = para_x_directriz_p_negativa(k , p) 

        elif p > 0:

            # Se encarga de formar la ecuación de la directriz de la parábola
            directriz = para_x_directriz_p_positiva(k , p)

    elif bandera == 'y2':

        if p < 0:

            directriz = para_y_directriz_p_negativa(h , p)

        elif p > 0:

            directriz = para_y_directriz_p_positiva(h , p)
...
def interseccion_recta_parabola(recta , parabola):
    from clase_geo_Gral import SegundoGrado
    ...
    if bandera == 'x2':
        ...
        if A != 0 and B != 0:
            ...
            #print(f'Determinante: {ecuacion.determinante()}\n')

            if ecuacion.determinante() >= 0:
